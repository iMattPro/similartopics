...
class driver_test extends \phpbb_database_test_case
{
	...
	public function test_get_query()
	{
		...
		if ($this->db->get_sql_layer() === 'postgres')
		{
			$select = "f.forum_id, f.forum_name, t.*, ts_rank_cd(to_tsvector('simple', t.topic_title), 'foo|bar', 32) AS score";
			$where = "ts_rank_cd(to_tsvector('simple', t.topic_title), 'foo|bar', 32) >= 0 AND t.topic_status <> 2 AND t.topic_visibility = 1 AND t.topic_time > (extract(epoch from current_timestamp)::integer - 0) AND t.topic_id <> 1";
		}
		else
		{
